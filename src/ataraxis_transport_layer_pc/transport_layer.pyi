...
from .helper_modules import (
    ...
    CompiledCRCProcessor as CompiledCRCProcessor,
    CompiledCOBSProcessor as CompiledCOBSProcessor,
)

_ZERO: np.uint8
_POLYNOMIAL: np.uint8
_EMPTY_ARRAY: NDArray[np.uint8]
_MINIMUM_SERIAL_BUFFER_SIZE: int
...
class TransportLayer:
    ...
    def _write_scalar_data(
        target_buffer: NDArray[np.uint8], scalar_object: Any, start_index: int, max_payload_size: int
    ) -> int: ...
    @staticmethod
    def _write_array_data(
        target_buffer: NDArray[np.uint8], array_object: NDArray[Any], start_index: int, max_payload_size: int
    ) -> int: ...
    @staticmethod
